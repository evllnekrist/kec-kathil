...
<div class="row">
  <div class="col-md-6">
    <label>Tanggal</label>
    <input type="text" name="tanggal" class="form-control tanggal" placeholder="Tanggal publikasi" value="<?php if(isset($_POST['tanggal'])) { echo set_value('tanggal'); }else{ echo date('Y-m-d',strtotime($download->tanggal)); } ?>" data-date-format="dd-mm-yyyy">
  </div>
  <div class="col-md-6">
  <label>Jam </label>
  <input type="text" name="jam" class="form-control time-picker" placeholder="Jam publikasi" value="<?php if(isset($_POST['jam'])) { echo set_value('jam'); }else{ echo date('H:i:s',strtotime($download->tanggal)); } ?>">
  </div>
</div>
</div>
