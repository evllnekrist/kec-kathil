...
class Nav_model extends CI_Model {
	...
	public function nav_topik2() {
		$this->db->select('*');
		$this->db->from('berita');
		$this->db->where_not_in('id_kategori', '11');
		$this->db->where(array(	'jenis_berita'	=> 'Berita',
								'status_berita'	=> 'Publish'));
		$this->db->order_by('tanggal_publish','DESC');
		$this->db->limit(1);
	...
	public function nav_topik3() {
		$this->db->select('berita.*, 
					users.nama');
		$this->db->from('berita');
		$this->db->join('users','users.id_user = berita.id_user','LEFT');
		$this->db->where('berita.id_kategori', '11');
		$this->db->where(array(	'jenis_berita'	=> 'Berita',
								'status_berita'	=> 'Publish'));
		$this->db->order_by('tanggal_publish','DESC');
		$this->db->limit(1);
	...
	public function nav_topik4() {
		$db2 = $this->load->database('db2', TRUE);
		$db2->select('berita.*, 
					users.nama');
		$db2->from('berita');
		$db2->join('users','users.id_user = berita.id_user','LEFT');
		$db2->where('berita.id_kategori', '11');
		$db2->where(array(	'jenis_berita'	=> 'Berita',
								'status_berita'	=> 'Publish'));
		$db2->order_by('tanggal_publish','DESC');
		$db2->limit(6);
		$query = $db2->get();
		return $query->result();
	}
