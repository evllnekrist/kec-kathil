...
class Agenda_model extends CI_Model {
	...
	$query = $this->db->get_where('agenda', array('sha1(id_agenda)' => $id_agenda));
	return $query->row_array();
	}
	...
	if ($id_agenda === FALSE)	{
		$query = $this->db->query('SELECT * FROM agenda ORDER BY id_agenda DESC LIMIT 3');
		return $query->result_array();
	}
	$query = $this->db->get_where('agenda', array('sha1(id_agenda)' => $id_agenda));
	return $query->row_array();
	...
	// Daftar agenda 
	public function data_agenda($agd) {
		$this->db->select('*');
		$this->db->from('agenda');
